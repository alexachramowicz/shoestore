class LineitemsController < ApplicationController
  ...
  def create
    ...
    product = Product.find(params[:product_id]) # pass product id to controller from shopper view
    #@lineitem = @cart.lineitems.build(product:product) # creates row in table
  ...
  def update
    respond_to do |format|
      if @lineitem.update(lineitem_params)
        # redirects to show cart (cart/id)
